...
function isUserApologizing(text) {
  const apolKeywords = ['آسف', 'أسف', 'اعتذر', 'أعتذر', 'سامحيني', 'سامحنى', 'حقك عليا', 'حقك علي', 'متقصدش', 'متزعلش', 'متازعلش', 'sorry', 'سامحني'];
...
function recordInteraction(reply, mood, dataType, replyDisplay = '') {
  ...
  globalState.last_reply_display = enforceEnglishScreenText(replyDisplay, "Lola: Ready!");
...
function setAnnoyedState() {
  globalState.annoyed_until = Date.now() + (60 * 60 * 1000); // 1 hour memory
...
function getMoodState() {
  ...
  return {
    ...
    last_reply_display: enforceEnglishScreenText(globalState.last_reply_display, "Lola: Ready!"),
    data_type: globalState.data_type,
    msg_id: globalState.msg_id
  };
}
