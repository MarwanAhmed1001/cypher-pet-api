// In-memory mood accumulator & state store for Cypher API
const VALID_MOODS = ["HAPPY", "NEUTRAL", "BORED", "SAD", "ANNOYED", "EXCITED"];

function cleanEnglishText(text, fallback = "Cypher: Ready!") {
...
let globalState = {
  last_reply: "أنا سايفر. عايز إيه؟",
  last_reply_display: "Cypher: Ready!",
...
function recordInteraction(reply, mood, dataType, replyDisplay = '', energyDelta = 0) {
  ...
  globalState.last_reply_display = cleanEnglishText(replyDisplay, "Cypher: Ready!");
...
function setAnnoyedState() {
  ...
  globalState.annoyed_until = Date.now() + (30 * 60 * 1000);
...
function getMoodState() {
  ...
  return {
    ...
    last_reply_display: cleanEnglishText(globalState.last_reply_display, "Cypher: Ready!"),
    data_type: globalState.data_type,
    msg_id: globalState.msg_id,
