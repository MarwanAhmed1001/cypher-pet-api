...
function recordInteraction(reply, mood, dataType, replyDisplay = '') {
  ...
  globalState.last_interaction_time = Date.now(); // Record exact timestamp
...
function getMoodState() {
  ...
  // If 15 seconds pass without new chat message, reset mood to NEUTRAL automatically
  const isRecentChat = (Date.now() - globalState.last_interaction_time < 15000);

  return {
    mood: isRecentChat ? globalState.mood : "NEUTRAL",
    daily_mood: globalState.daily_mood,
    last_reply: isRecentChat ? globalState.last_reply : "أهلاً بك! أنا لولا.",
    last_reply_display: isRecentChat ? globalState.last_reply_display : "Lola: Ready!",
    data_type: globalState.data_type
  };
}
